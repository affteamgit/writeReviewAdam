...
def generate_overview_section(casino_name, keyword, main_points, review_content=None):
    """Generate Overview section with TLDR using Adam's fine-tuned model."""
    try:
        print("Generating Overview section with TLDR using Adam's voice...")

        # Create prompt for overview generation that includes TLDR
        if review_content:
            overview_prompt = f"""Write an engaging overview/introduction for a {casino_name} casino review that includes a TLDR section. Use the following details:
...
Write a compelling overview that contains:
1. A 2-3 paragraph introduction that:
   - MUST include the exact phrase "{keyword}" somewhere (verbatim for SEO purposes)
   - Touches on the main points provided
   - Sets expectations for what the full review will cover
   - Maintains your signature analytical and honest approach

2. A TLDR section at the end that:
   - Summarizes the key findings from the full review content
   - Highlights the most important pros and cons
   - Provides a clear verdict on whether this casino is recommended
   - Uses bullet points for easy scanning (3-5 bullet points maximum)
   - Is formatted as "**TLDR**" followed by bullet points starting with "-"
...
Format the response with the introduction paragraphs first, then the TLDR section."""
        else:
            # Fallback for when review content isn't available yet
...
Do not repeat information that will be covered in detail in other sections - this should be a high-level introduction that draws readers in."""
        ...
        print("Successfully generated Overview section with TLDR")
        return f"**Overview**\n{overview_content}"
        ...
        with col1:
            if st.button("Generate Overview & Post to Google Docs", type="primary", disabled=not (keyword and main_points)):
                if keyword and main_points:
                    try:
                        # Generate overview section with TLDR
                        st.info("🔄 Generating Overview section with TLDR using Adam's voice...")
                        ...
                        # Combine overview (which now includes TLDR) with the rest of the review
